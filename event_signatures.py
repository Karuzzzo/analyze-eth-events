...
def generate_event_signatures(w3):
    ...
    all_sigs = {
        # 'aave_liquidation': aave_liquidation_event,
...
def add_to_event_sigs(w3, event_name, event_sig_text):
    event_sig = w3.keccak(text=event_sig_text).hex()
    event_sigs = get_event_signatures()
    event_sigs[event_name] = event_sig
